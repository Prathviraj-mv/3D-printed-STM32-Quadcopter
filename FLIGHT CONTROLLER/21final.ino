...
void motorMixing(int throttle) {
  ...
  motor2 = throttle + pitch_output + roll_output;  // Front Right
  motor3 = throttle + pitch_output - roll_output;  // Front Left
  motor4 = throttle - pitch_output - roll_output;  // Rear Left
  motor1 = throttle - pitch_output + roll_output;  // Rear Right
...
void setup() {
  ...
  delay(4000);  // Wait for ESCs to arm (3 seconds)
...
void loop() {
  ...
  int throttle = map(pwmValues[2], 1100, 2000, 1000, 1900);
